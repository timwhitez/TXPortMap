...
func ComparePackets(rcv []byte, rcvSize int, szBan *string, szSvcName *string) int {
	...
	for i := 0; i < rcvSize; i++ {
		if !strconv.IsPrint(rune(buf[i])) {
			printBuf.WriteString(fmt.Sprintf("\\x%02x", buf[i])) // 不可打印字符
		} else {
			printBuf.WriteByte(buf[i]) // 可打印字符
		}
	}
	var bufUp = bytes.ToUpper([]byte(printBuf.String()))
	...
	if len(buf) >= 8 {
		if rcvSize > 7 && buf[4] == 0xff && buf[0] == uint8(rcvSize-4) {
			*szSvcName = "mysql"
			uErr := binary.LittleEndian.Uint16(buf[5:7])
			if uErr == 1129 {
				*szBan = "BLOCKED"
	...
	if len(buf) >= 10 {
		if rcvSize > 5 && buf[4] >= 10 {
			index := strings.Index(printBuf.String(), "\\x00\\x00\\x00\\x0a")
			if index != -1 {
				right := printBuf.String()[len("\\x00\\x00\\x00\\x0a")+index:]
	...
	// ROS TODO::
	if strings.Contains(printBuf.String(), "\\x13\\x02list") {
		*szSvcName = "MikroTik"
	...
	if len(buf) >= 17 {
		if bytes.Equal(buf[:17], []byte("Zookeeper version")) {
			...
			*szBan = strings.Split(printBuf.String(), "-")[0]
			goto Return
		}
	}

	// Java-RMI  TODO::
	if strings.Contains(printBuf.String(), "|com.code42.messaging.security.") {
		*szSvcName = "JavaRMI"
	...
	if strings.Index(printBuf.String(), "\\xac\\xed\\x00\\x05") == 0 {
		*szSvcName = "JavaRMI"
		*szBan = printBuf.String()
	...
	// jabber
	if len(buf) >= len("<stream:error xmlns") {
	...
	if len(buf) >= 5 {
		if rcvSize > 5 && bytes.Equal(buf[:5], []byte("HELO:")) {
			*szSvcName = "WebLogic-t3"
			...
			// TODO::
			n := strings.Index(printBuf.String(), "\\x0a")
			if n != -1 {
				bantmp := printBuf.String()[:n]
	...
	if bytes.Index(bufUp, []byte("HTTP/")) == 0 || bytes.Index(bufUp, []byte("RTSP/1")) == 0 {
		...
		n := strings.Index(printBuf.String(), "\\x0d\\x0aServer:")
		if n != -1 {
			bantmp := printBuf.String()[len("\\x0d\\x0aServer:")+n:]
		...
		if len(printBuf.String()) > 100 {
			*szBan = printBuf.String()[:100]
		} else {
			*szBan = printBuf.String()
		}
		goto Return
	}
	if strings.EqualFold(printBuf.String(), "\\x01\\x00\\x00\\x00") {
		dwRecognition = UNKNOWN_SVC
		*szSvcName = "bandwidth-test"
		*szBan = "MikroTik bandwidth-test server"
		goto Return
	}
	if strings.EqualFold(printBuf.String(), "BadRecord") {
		dwRecognition = UNKNOWN_SVC
		*szSvcName = "h.239"
		*szBan = "Polycom People+Content IP H.239"
		goto Return
	}
	if strings.EqualFold(printBuf.String(), "\\x03\\x00\\x000\\x08\\x02\\x00\\x00}\\x08\\x02\\x80\\xe2\\x14\\x01\\x00~\\x00\\x1d\\x05\\x08 \\x19\\x00\\x06\\x00\\x08\\x91J\\x00\\x05\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00") {
		dwRecognition = UNKNOWN_SVC
		*szSvcName = "h323q931"
		*szBan = "Polycom ViewStation H.323"
		goto Return
	}
	if len(buf) >= len(os___xrdp_1) {
		if buf[0] == 0x03 && buf[1] == 0x00 && buf[2] == 0x00 {
			dwRecognition = RDP
			*szSvcName = "rdp"
			if bytes.Equal(buf[:len(os______old)], os______old) {
				*szBan = "Windows Server 2003 or before"
			} else if bytes.Equal(buf[:len(os___xrdp_1)], os___xrdp_1) || bytes.Equal(buf[:len(os___xrdp_2)], os___xrdp_2) || bytes.Equal(buf[:len(os___xrdp_3)], os___xrdp_3) || bytes.Equal(buf[:len(os___xrdp_4)], os___xrdp_4) {
				*szBan = "xrdp"
			} else if bytes.Equal(buf[:len(os___2008_1)], os___2008_1) || bytes.Equal(buf[:len(os___2008_2)], os___2008_2) || bytes.Equal(buf[:len(os___2008_3)], os___2008_3) {
				*szBan = "Windows Server 2008 [R2] [Standard/Enterprise/Datacenter]"
			} else if bytes.Equal(buf[:len(os___2012_1)], os___2012_1) || bytes.Equal(buf[:len(os___2012_2)], os___2012_2) || bytes.Equal(buf[:len(os__2012_r2)], os__2012_r2) {
				*szBan = "Windows Server 2012 [R2]"
			} else if bytes.Equal(buf[:len(os__Vista_1)], os__Vista_1) {
				*szBan = "Windows Vista or later"
			} else if bytes.Equal(buf[:len(os_Multiple_1)], os_Multiple_1) || bytes.Equal(buf[:len(os_Multiple_2)], os_Multiple_2) {
	...
	if bytes.Contains(buf, []byte("Y(DESCRIPTION=(TMP=)(VSNNUM=")) || bytes.Contains(buf, []byte(")(ERROR_STACK=(ERROR=(CODE=")) {
		// TODO
		if n := strings.Index(printBuf.String(), "(VSNNUM="); n != -1 {
			buftmp := printBuf.String()[n+8:]
	...
	if n := strings.Index(printBuf.String(), "TNSLSNR"); n != -1 {
		// 允许TNSLSNR查询
		// TODO::
		buftmp := printBuf.String()[len(printBuf.String())-n:]
	...
	if bytes.Equal(buf[:len("+PONG")], []byte("+PONG")) {
		...
		*szSvcName = "redis"
		// TODO::
		if n := strings.Index(printBuf.String(), "redis_version:"); n != -1 {
			buftmp := printBuf.String()[len(printBuf.String())-n-14:]
	...
	if bytes.Contains(buf, []byte("-NOAUTH Authentication required")) || bytes.Contains(buf, []byte("-ERR operation not permitted")) || bytes.Contains(buf, []byte("-ERR wrong number of arguments for '")) || bytes.Contains(buf, []byte("-ERR unknown command '")) || bytes.Contains(buf, []byte("-ERR unknown command `")) {
	...
	// memcached
	if len(buf) >= len("STAT pid ") {
		if bytes.Equal(buf[:len("STAT pid ")], []byte("STAT pid ")) {
			*szSvcName = "memcached"
			dwRecognition = MEMCACHED
			// TODO: 获取版本号
			if n := strings.Index(printBuf.String(), "STAT version "); n != -1 {
				buftmp := printBuf.String()[len(printBuf.String())-n-13:]
				if idx := strings.Index(buftmp, "\\x0d\\x0a"); idx != -1 {
					*szBan = buftmp[:idx]
					goto Return
				}
	...
	if len(buf) >= 4 {
		...
		if bytes.Equal(buf[:4], cBit_MongoDB) && bytes.Equal(buf[8:12], cFlag_MongoDB) {
			if bytes.Contains(buf, []byte("host")) && bytes.Contains(buf, []byte("version")) && bytes.Contains(buf, []byte("uptime")) && bytes.Contains(buf, []byte("ok")) {
				*szSvcName = "mongodb"
		...
		if bytes.EqualFold(buf[:4], []byte("SIP/")) {
			*szSvcName = "sip"
			...
			if n == -1 {
				...
				n = strings.Index(printBuf.String(), "\\x0d\\x0aUser-Agent:")
			}
			if n != -1 {
				buftmp := printBuf.String()[len(printBuf.String())-n-len(mark):]
				if idx := strings.Index(buftmp, "\\x0d\\x0a"); idx != -1 {
					*szBan = buftmp[:idx]
					goto Return
				}
	...
	// POSTGRESQL
	if len(buf) >= 5 {
		if buf[0] == 0x45 && buf[4] == byte(rcvSize-1) && (bytes.Contains(buf, []byte("FATAL")) || bytes.Contains(buf, []byte("Fpostmaster.c"))) {
	...
	if len(buf) >= 5 {
		if buf[0] == 0x52 && (buf[4] == byte(rcvSize-1) || bytes.Contains(buf, []byte("server_version\x00")) || bytes.EqualFold(buf[:len("R\\x00\\x00\\x00\\x08\\x00\\x00\\x00\\x00")], []byte("R\\x00\\x00\\x00\\x08\\x00\\x00\\x00\\x00"))) {
			*szSvcName = "postgresql"
			dwRecognition = POSTGRESQL
			goto Return
		}
	...
	// SVRLOC
	if len(buf) >= 16 {
		if bytes.Contains(buf, []byte("service:service-agent://")) || (buf[0] == 0x02 && buf[1] == 0x0b && buf[13] == 0x02 && buf[14] == 0x65 && buf[15] == 0x6e) {
			*szSvcName = "svrloc"
	...
	// AJP13
	if strings.EqualFold(printBuf.String(), "AB\\x00\\x01\\x09") {
		*szSvcName = "ajp13"
		dwRecognition = AJP13
	...
	//fmt.Println(rcvSize)
	if strings.EqualFold(printBuf.String(), "AB\\x00") {
		*szSvcName = "ajp13"
		dwRecognition = AJP13
		*szBan = "Apache Jserv"
		goto Return
	...
	// nfs TODO::
	if strings.Index(printBuf.String(), "\\x80\\x00\\x00") == 0 && strings.Contains(printBuf.String(), "\\x10l\\x8e") {
		*szSvcName = "nfs"
		dwRecognition = NFS
		goto Return
	}
	...
	if strings.Index(printBuf.String(), "\\x84\\x00\\x00\\x00") == 0 {
	...
	// TELNET
	if bytes.Contains(buf, []byte("ogin:")) || bytes.Contains(buf, []byte("ccount:")) || bytes.Contains(buf, []byte("ser:")) || bytes.Contains(buf, []byte("ame:")) || bytes.Contains(buf, []byte("assword:")) || bytes.Contains(buf, []byte("telnet")) || bytes.Contains(buf, []byte("System administrator")) || bytes.Contains(buf, []byte("%connection closed by remote host!")) || bytes.Contains(buf, []byte("not allowed now, you may try")) || bytes.Contains(buf, []byte("All user interfaces are used, please try later")) || (buf[0] == 0xff && buf[1] == 0xfd) || (buf[0] == 0xff && buf[1] == 0xfb) {
	...
	// elasticsearch
	if strings.EqualFold(printBuf.String(), "This is not a HTTP port") {
	...
	// bgp
	if strings.Index(printBuf.String(), "\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff") == 0 {
		*szSvcName = "bgp"
		dwRecognition = UNKNOWN_SVC
		goto Return
	}
	...
	// ldp
	if strings.Index(printBuf.String(), "\\x00\\x01\\x00\\x1c\\x00\\x00\\x00\\x00\\x00\\x00") == 0 {
		*szSvcName = "ldp"
		dwRecognition = UNKNOWN_SVC
		goto Return
	}
	...
	// xmpp
	if strings.EqualFold(printBuf.String(), "</stream:stream>") {
		*szSvcName = "xmpp"
		dwRecognition = UNKNOWN_SVC
		*szBan = printBuf.String()
		goto Return
	...
	if strings.Index(printBuf.String(), "\\x00\\x00\\x00\\x10\\x00\\x00\\x00") == 0 {
		dwRecognition = UNKNOWN_SVC
		*szSvcName = "ipcam"
		*szBan = printBuf.String()
	...
	// shoutcast
	if strings.EqualFold(printBuf.String(), "invalid password\\x0d\\x0a") {
		dwRecognition = UNKNOWN_SVC
		*szSvcName = "shoutcast"
	...
	// zebra
	if strings.EqualFold(printBuf.String(), "Vty password is not set.\\x0d\\x0a") {
		dwRecognition = UNKNOWN_SVC
		*szSvcName = "zebra"
...
func IntToBytes(n int, b byte) ([]byte, error) {
	...
	return nil, fmt.Errorf("IntToBytes b param is invalid")
}
