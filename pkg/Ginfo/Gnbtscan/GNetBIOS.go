...
//forked from https://github.com/shadow1ng/fscan/blob/main/Plugins/NetBIOS.go
...
//Scan get netbios stat information by udp137
func Scan(ip string) (string, error) {
	nbname, err := GetNbnsname(ip)
	var msg, isdc string

	if strings.Contains(nbname.msg, "Domain Controllers") {
		isdc = "[DC]"
	}
	msg = fmt.Sprintf("%s%s\\%s   %s", isdc, nbname.group, nbname.unique, nbname.osversion)
...
//NetBIOS1 get netbios name, group and os version by udp137 and tcp139 port
func NetBIOS1(ip string) (nbname NbnsName, err error) {
	nbname, err = GetNbnsname(ip)
	var payload0 []byte
	if err == nil {
	...
	realhost := fmt.Sprintf("%s:%v", ip, 139)
...
func GetNbnsname(ip string) (nbname NbnsName, err error) {
